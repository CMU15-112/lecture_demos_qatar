...
def selectedCell(app, x, y):
    ...
    cellW, cellH = (app.width)//app.ncols, (app.height)//app.nrows  
    for row in range(app.nrows):
        for col in range(app.ncols):
            # compute coordinates
            x1 = cellW * col
...
def drawGrid(width, height, cellColors, nrows, ncols):
    cellW, cellH = (width)//ncols, (height)//nrows  
    for row in range(nrows):
        for col in range(ncols):
            ...
            # draw the rectangle
            drawRect(x1, y1, cellW, cellH, fill=curColor)
...
Press s to start"""
        ...
        drawLabel(f"Time Remaining: {app.countDown}", 0, fontSize//7, \
            font="Arial", size=fontSize//7, bold=True, align='left-top')
