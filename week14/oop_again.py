class Book:
    def __init__(self, title, author, year, copies):
        ...
        self.copies = copies
    def __repr__(self):
        return f'{self.author}, "{self.title}", {self.year}, {self.copies}'
...
assert(str(b1) == """Donald Knuth, "The Art of Computer Programming", 1968, 5""")
...
assert(str(j1) == """Various Authors, "Science Vol 374 Issue 6568", 2021, 8""")

assert(j1 != b1)
assert(isinstance(j1, Book))
assert(type(j1) != type(b1))
